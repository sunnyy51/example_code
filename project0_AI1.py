...
#level1
#AI moves to a spot that is generated by getLongSeq function 
def AIMove_lv1(board,k, symbol):
    m = len(board)
    n = len(board[0])
    curr_max=0 
    next_x=-1
    ...
    for i in range(m):
        for j in range(n):
            if board[i][j] == CHECK_X:
              curr_pos_list = getLongSeq(board, i, j, k, opponent_symbol )
              if curr_pos_list[0] > curr_max:
                  next_x = curr_pos_list[1]
                  next_y = curr_pos_list[2]
    return [next_x, next_y]
              
#gets longest sequence of AI marks, calls the four directional longest marks functions                
...
while True:
    ...
    if isWinner(bo, move_x, move_y, k, CHECK_O) == True: 
        ...
        break  
    #AI move
    [move_x,move_y] = AIMove_lv1(bo, k, CHECK_X)  
